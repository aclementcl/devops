Vagrant.configure("2") do |config|
  ...
  config.vm.provision :shell do |shell| # Install docker dependencies
      shell.inline = <<-SHELL
  ...
  #config.vm.provision :shell do |shell| // If you wish to use shell inline commands against the VM
   #   shell.inline = <<-SHELL
